...
(deftest test-find*
  (is (= "Moustache" (text (find-it b :a))))
  (is (= "Moustache" (text (find-it b {:class "external"}))))
  (is (= "https://github.com/cgrand/moustache" (attribute (find-it b {:text "Moustache"}) "href")))
  (is (= "Moustache" (text (find-it b :a {:class #"exter"}))))
  (is (= "Moustache" (text (find-it b :a {:text #"Mous"}))))
  (is (= "Moustache" (text (find-it b :a {:class "external", :href #"github"}))))
  ...
  (is (= "Moustache" (text (find-it b [:div {:id "content"}, :a {:href #"github"}]))))
  (is (= "home"      (text (find-it b [:* {:id "footer"}, :a {}]))))
  (is (= 4           (count (find-them b :a))))
