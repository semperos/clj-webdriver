...
(deftest test-missing-elements
  (let [missing "i#dont.exist"]
    (is (= nil (element missing)))
    (is (= [] (elements missing)))))

(deftest test-missing-elements-composite-fns
  (let [missing "i#dont.exist"]
    (is (= nil (find-elements-under missing (element "a"))))
    (is (= nil (find-element-under missing (element "a"))))
    (is (= nil (attribute missing :text)))
    (is (= nil (click missing)))
    (is (= nil (displayed? missing)))
    (is (= nil (drag-and-drop missing missing)))
    (is (= nil (drag-and-drop-by missing {:x -15 :y -5})))
    (is (= nil (exists? missing)))
    (is (= nil (flash missing)))
    (is (= nil (focus missing)))
    (is (= nil (location missing)))
    (is (= nil (location-once-visible missing)))
    (is (= nil (present? missing)))
    (is (= nil (element-size missing)))
    (is (= nil (intersects? missing missing)))
    (is (= nil (tag missing)))
    (is (= nil (text missing)))
    (is (= nil (value missing)))
    (is (= nil (visible? missing)))
    (is (= nil (xpath "#login_field")))
    (is (= nil (enabled? missing)))
    (is (= nil (input-text missing "abc")))
    (is (= nil (submit missing)))
    (is (= nil (clear missing)))
    (is (= nil (select missing)))
    (is (= nil (selected? missing)))
    (is (= nil (send-keys missing "abc")))
    (is (= nil (toggle missing)))
    (is (= nil (options missing)))
    (is (= nil (selected-options missing)))
    (is (= nil (deselect-option missing {:value "foo"})))
    (is (= nil (deselect-all missing)))
    (is (= nil (deselect-by-index missing 1)))
    (is (= nil (deselect-by-value missing "foo")))
    (is (= nil (multiple? missing)))
    (is (= nil (select-option missing {:value "foo"})))
    (is (= nil (select-all missing)))
    (is (= nil (select-by-index missing 1)))
    (is (= nil (select-by-text missing "foo")))
    (is (= nil (select-by-value missing "foo")))))