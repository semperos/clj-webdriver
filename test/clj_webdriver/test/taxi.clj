...
(deftest-template-param
  ...
  (is (= (attribute __ "input[name='first_name']" :id) "first_name"))
  (is (= (attribute __ "input[type='text'][name='first_name']" :id) "first_name"))
  (is (= (attribute __ "input[type='text'][name^='first_']" :id) "first_name"))
  (is (= (text __ "a") "home"))
  (is (= (text __ "a.menu-item") "home"))
  (is (= (attribute __ {:tag "a", :class "menu-item"} :class) "menu-item"))
  (is (= (text __ "#footer a.menu-item") "home"))
  (is (= (attribute __ "option[value*='cial_']" :value) "social_media"))
  (is (= (attribute __ "option[value^='social_']" :value) "social_media"))
  (is (= (attribute __ "option[value$='_media']" :value) "social_media"))
  (is (= (attribute __ "option[value]" :value) "france"))
  (back __) ;; on starting page
  (is (= (attribute __ "*.first.odd" :class) "first odd"))
  (is (= (tag __ {:class "external"}) "a"))
  (is (= (text __ (nth (elements __ "a") 1)) "Moustache"))
  (is (= (text __ "*.external") "Moustache"))
  (is (= (attribute __ "*.first.odd" :class) "first odd"))
  (is (= (attribute __ "li.first.odd" :class) "first odd"))
  (is (= (count (elements __ "a")) 10))
  (is (= (text __ "a[class^='exter']") "Moustache"))
  (is (= (text __ "a.external[href*='github']") "Moustache"))
  (is (= (text __ "a[class*='exter'][href*='github']") "Moustache"))
  (is (= (count (elements __ "*[class*='-item']")) 5))
  (is (= (count (elements __ "a[class*='-item']")) 5))
